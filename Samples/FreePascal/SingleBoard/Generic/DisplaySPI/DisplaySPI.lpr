...
}
{
  ...
  Attention! Please follow these instructions before running the sample:
   ...
   1. When creating TSysfsGPIO and TSysfsSPI classed, make sure that the corresponding Linux paths are correct.
      For instance, on BeagleBoneBlack they are "/sys/class/gpio" and "/dev/spidev1.0" accordingly.

   2. PinRST and PINDC constants should contain actual Linux GPIO numbers corresponding to physical pins.
...
uses
  Crt, SysUtils, PXL.TypeDef, PXL.Types, PXL.Fonts, PXL.Boards.Types, PXL.Sysfs.GPIO, PXL.Sysfs.SPI,
  PXL.Displays.Types, PXL.Displays.SSD1351;

const
  // Please make sure to specify the following pins according to Linux GPIO numbering scheme on your device.
  PinRST = 60;
  PinDC = 48;
