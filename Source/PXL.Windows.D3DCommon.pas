...
const
  ...
  SID_ID3D10Blob = '{8BA5FB08-5195-40E2-AC58-0D989C3A0102}';
  IID_ID3D10Blob: TGuid = SID_ID3D10Blob;
  IID_ID3DBlob: TGuid = SID_ID3D10Blob;

type
  PD3D_DRIVER_TYPE = ^D3D_DRIVER_TYPE;
  D3D_DRIVER_TYPE = LongWord;

  PD3D_FEATURE_LEVEL = ^D3D_FEATURE_LEVEL;
  D3D_FEATURE_LEVEL = LongWord;

  PD3D_PRIMITIVE_TOPOLOGY = ^D3D_PRIMITIVE_TOPOLOGY;
  D3D_PRIMITIVE_TOPOLOGY = LongWord;

  PD3D_PRIMITIVE = ^D3D_PRIMITIVE;
  D3D_PRIMITIVE = LongWord;

  PD3D_SRV_DIMENSION = ^D3D_SRV_DIMENSION;
  D3D_SRV_DIMENSION = LongWord;

  PD3D_SHADER_MACRO = ^D3D_SHADER_MACRO;
  D3D_SHADER_MACRO = record
    Name: PAnsiChar;
    Definition: PAnsiChar;
  end;

  PD3D_INCLUDE_TYPE = ^D3D_INCLUDE_TYPE;
  D3D_INCLUDE_TYPE = LongWord;

  PD3D_SHADER_VARIABLE_CLASS = ^D3D_SHADER_VARIABLE_CLASS;
  D3D_SHADER_VARIABLE_CLASS = LongWord;

  PD3D_SHADER_VARIABLE_FLAGS = ^D3D_SHADER_VARIABLE_FLAGS;
  D3D_SHADER_VARIABLE_FLAGS = LongWord;

  PD3D_SHADER_VARIABLE_TYPE = ^D3D_SHADER_VARIABLE_TYPE;
  D3D_SHADER_VARIABLE_TYPE = LongWord;

  PD3D_SHADER_INPUT_FLAGS = ^D3D_SHADER_INPUT_FLAGS;
  D3D_SHADER_INPUT_FLAGS = LongWord;

  PD3D_SHADER_INPUT_TYPE = ^D3D_SHADER_INPUT_TYPE;
  D3D_SHADER_INPUT_TYPE = LongWord;

  PD3D_SHADER_CBUFFER_FLAGS = ^D3D_SHADER_CBUFFER_FLAGS;
  D3D_SHADER_CBUFFER_FLAGS = LongWord;

  PD3D_CBUFFER_TYPE = ^D3D_CBUFFER_TYPE;
  D3D_CBUFFER_TYPE = LongWord;

  PD3D_NAME = ^D3D_NAME;
  D3D_NAME = LongWord;

  PD3D_RESOURCE_RETURN_TYPE = ^D3D_RESOURCE_RETURN_TYPE;
  D3D_RESOURCE_RETURN_TYPE = LongWord;

  PD3D_REGISTER_COMPONENT_TYPE = ^D3D_REGISTER_COMPONENT_TYPE;
  D3D_REGISTER_COMPONENT_TYPE = LongWord;

  PD3D_TESSELLATOR_DOMAIN = ^D3D_TESSELLATOR_DOMAIN;
  D3D_TESSELLATOR_DOMAIN = LongWord;

  PD3D_TESSELLATOR_PARTITIONING = ^D3D_TESSELLATOR_PARTITIONING;
  D3D_TESSELLATOR_PARTITIONING = LongWord;

  PD3D_TESSELLATOR_OUTPUT_PRIMITIVE = ^D3D_TESSELLATOR_OUTPUT_PRIMITIVE;
  D3D_TESSELLATOR_OUTPUT_PRIMITIVE = LongWord;

  PD3D_MIN_PRECISION = ^D3D_MIN_PRECISION;
  D3D_MIN_PRECISION = LongWord;

  PD3D_INTERPOLATION_MODE = ^D3D_INTERPOLATION_MODE;
  D3D_INTERPOLATION_MODE = LongWord;

  PFourSingleArray = ^TFourSingleArray;
  TFourSingleArray = array[0..3] of Single;

  PFourLongWordArray = ^TFourLongWordArray;
  TFourLongWordArray = array[0..3] of LongWord;
  ...
  ID3DInclude = interface(IUnknown)
    function Open(IncludeType: D3D_INCLUDE_TYPE; FileName: PAnsiChar; ParentData, Data: Pointer;
      out Bytes: LongWord): HRESULT; stdcall;
    function Close(Data: Pointer): HResult; stdcall;
  end;

function FourSingleArray(const Value1, Value2, Value3, Value4: Single): TFourSingleArray; inline;

implementation

function FourSingleArray(const Value1, Value2, Value3, Value4: Single): TFourSingleArray;
begin
  Result[0] := Value1;
  Result[1] := Value2;
  Result[2] := Value3;
