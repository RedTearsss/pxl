...
type
  TFireDX11Provider = class(TGraphicsDeviceProvider)
  public
...
uses
  PXL.Devices.FM.DX11, PXL.Textures.DX11, PXL.Canvas.DX11;

function TFireDX11Provider.CreateDevice: TCustomDevice;
begin
  Result := TFireDX11Device.Create(Self);
end;

function TFireDX11Provider.CreateCanvas(const Device: TCustomDevice): TCustomCanvas;
begin
  Result := TDX11Canvas.Create(Device);
end;

function TFireDX11Provider.CreateLockableTexture(const Device: TCustomDevice;
  const AutoSubscribe: Boolean): TCustomLockableTexture;
begin
  Result := TDX11LockableTexture.Create(Device, AutoSubscribe);
end;

function TFireDX11Provider.CreateDrawableTexture(const Device: TCustomDevice;
